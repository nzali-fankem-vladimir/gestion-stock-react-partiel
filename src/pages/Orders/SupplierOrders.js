...
import { serviceCommandeFournisseur } from '../../services/api';
import BoutonAction from '../../components/ActionButton/ActionButton';
import Pagination from '../../components/Pagination/Pagination';
import './SupplierOrders.css';

/**
 * Page de gestion des commandes fournisseurs
 */
const CommandesFournisseurs = () => {
  ...
  const chargerCommandes = async () => {
    try {
      setChargement(true);
      const reponse = await serviceCommandeFournisseur.obtenirTous();
  ...
  const supprimerCommande = async (id) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer cette commande ?')) {
      try {
        await serviceCommandeFournisseur.supprimer(id);
  ...
  const commandesFiltrees = commandes.filter(commande =>
    ...
    commande.fournisseur?.nom?.toLowerCase().includes(recherche.toLowerCase())
  );
  ...
  return (
    <div className="page-commandes-fournisseurs">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted">Gérez vos commandes auprès des fournisseurs</p>
        </div>
        <Link to="/nouvellecommandefrs" className="btn btn-primary">
        ...
        <div className="card-body">
          {commandesAffichees.length === 0 ? (
            <div className="text-center py-5">
              <i className="fas fa-truck-loading fa-3x text-muted mb-3"></i>
                      ...
                      <tr key={commande.id}>
                        ...
                        <td>
                          <div>
                            ...
                            <small className="text-muted">{commande.fournisseur?.email}</small>
                          </div>
                        </td>
                        <td>
                        ...
                        <td>
                          <span className="fw-bold text-success">
                            {commande.ligneCommandeFournisseurs?.reduce((total, ligne) => 
                        ...
                        <td>
                          <BoutonAction
                            ...
                            onModifier={() => window.location.href = `/nouvellecommandefrs/${commande.id}`}
